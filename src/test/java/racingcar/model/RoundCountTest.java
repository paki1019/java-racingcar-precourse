...
class RoundCountTest {
	...
	void generate(String input) {
		RoundCount roundCount = RoundCount.generate(input);
		assertThat(roundCount).isNotNull();
		assertThat(roundCount.getRoundCount()).isEqualTo(Integer.parseInt(input));
	}
	...
	void generate_not_number(String input) {
		assertThatThrownBy(() -> RoundCount.generate(input))
